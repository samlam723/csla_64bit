...
# csla_64bit. The P&R flow is based on the EDI 13.1 official workshop and use the
...
set init_verilog "designs/csla_64bit.mapped.v"
...
floorPlan -r 1 0.6 20 20 24 24

# Load IO assignment file: csla_64bit.ioc
# It specifies the relative location of module pins and on which metal layer you
# are preferred to place your module pins
loadIoFile "scripts/csla_64bit.ioc" -noAdjustDieSize
# Save the checkpoint for floorplan
saveDesign db/csla_64bit_floorplan.enc
...
# starting from the left offset 20.0 (roughly middle of the csla_64bit module)
...
saveDesign db/csla_64bit_powerplan.enc
...
saveDesign db/csla_64bit_place.enc
...
saveDesign db/csla_64bit_cts.enc
...
saveDesign db/csla_64bit_postroute.enc
...
verifyGeometry -report reports/csla_64bit.geometry.rpt
verifyConnectivity -type all -report reports/csla_64bit.connectivity.rpt
...
report_area -out_file reports/csla_64bit.routed.area.rpt

# Power report
report_power -outfile reports/csla_64bit.routed.power.rpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/csla_64bit.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/csla_64bit.sdf

# *.v: verilog netlist file
saveNetlist results/csla_64bit.routed.v

# *.gds: GDS file
streamOut results/csla_64bit.gds -mapFile streamOut.map \
